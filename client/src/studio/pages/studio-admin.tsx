...
import {
  ...
  BarChart3, Film, Calendar, Mic2, Shield, Trash2, Download, LogOut
} from "lucide-react";
...
const StudioAdmin = memo(function StudioAdmin({ studioId }: { studioId: string }) {
  ...
  const [deleteStudioConfirm, setDeleteStudioConfirm] = useState(false);
  ...
  const [newProductionVideoUrl, setNewProductionVideoUrl] = useState("");
  const [newProductionIsPublic, setNewProductionIsPublic] = useState(false);
  const [newProductionScriptJson, setNewProductionScriptJson] = useState("");
  const [editProduction, setEditProduction] = useState<any | null>(null);
  const [editProductionForm, setEditProductionForm] = useState({ name: "", description: "", videoUrl: "", isPublic: false, scriptJson: "" });
  ...
  const [newSessionStatus, setNewSessionStatus] = useState("scheduled");
  ...
  const { data: studio } = useQuery({
    ...
    queryFn: () => authFetch(`/api/studios/${studioId}`),
  });
  ...
  const deleteStudioMutation = useMutation({
    mutationFn: () => authFetch(`/api/admin/studios/${studioId}`, { method: "DELETE" }),
    onSuccess: () => {
      toast({ title: "Estudio excluido com sucesso" });
      navigate("/hub-dub/studios");
    },
    onError: (err: any) => {
      toast({ title: "Erro ao excluir estudio", description: err?.message, variant: "destructive" });
    },
  });
  ...
  const approveMutation = useMutation({
    ...
    onError: (err: any) => {
      toast({ title: "Erro ao aprovar membro", description: err?.message, variant: "destructive" });
    },
  });
  ...
  const updateRolesMutation = useMutation({
    ...
    onError: (err: any) => {
      toast({ title: "Erro ao atualizar papeis", description: err?.message, variant: "destructive" });
    },
  });
  ...
  const createProductionMutation = useMutation({
    mutationFn: async () => {
      return authFetch(`/api/studios/${studioId}/productions`, {
        ...
        body: JSON.stringify({
          ...
          description: newProductionDesc,
          videoUrl: newProductionVideoUrl || null,
          isPublic: newProductionIsPublic,
          scriptJson: newProductionScriptJson.trim() || null,
          studioId,
        }),
      });
    },
    onSuccess: () => {
      ...
      setNewProductionName(""); setNewProductionDesc(""); setNewProductionVideoUrl(""); setNewProductionIsPublic(false); setNewProductionScriptJson("");
      toast({ title: "Producao criada" });
    },
    onError: (err: any) => {
      toast({ title: "Erro ao criar producao", description: err?.message, variant: "destructive" });
    },
  });
  ...
  const createSessionMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewSessionTitle(""); setNewSessionProductionId(""); setNewSessionDate(""); setNewSessionDuration("60"); setNewSessionStatus("scheduled");
      toast({ title: "Sessao criada" });
    },
    onError: (err: any) => {
      toast({ title: "Erro ao criar sessao", description: err?.message, variant: "destructive" });
    },
  });
  ...
  const filteredMembers = memberSearch.trim()
    ? approvedMembers.filter((m: any) =>
        (m.user?.fullName || m.user?.displayName || m.user?.email || "").toLowerCase().includes(memberSearch.toLowerCase()) ||
  ...
  return (
    <PageSection>
      <PageHeader
        label="Administracao"
        title={(studio as any)?.name || "Painel do Estudio"}
          ...
          <>
            {user?.role === "platform_owner" && (
              <Button
                ...
                onClick={() => setDeleteStudioConfirm(true)}
      ...
      <Dialog open={deleteStudioConfirm} onOpenChange={setDeleteStudioConfirm}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Excluir Estudio</DialogTitle>
            <DialogDescription>
              Tem certeza que deseja excluir permanentemente <strong>{(studio as any)?.name}</strong>?{" "}
              Todas as producoes, sessoes e takes deste estudio serao removidos. Esta acao nao pode ser desfeita.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteStudioConfirm(false)}>Cancelar</Button>
            <Button
              variant="destructive"
              onClick={() => { setDeleteStudioConfirm(false); deleteStudioMutation.mutate(); }}
              disabled={deleteStudioMutation.isPending}
            >
              {deleteStudioMutation.isPending ? "Excluindo..." : "Excluir Estudio"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <div className="flex-1 min-w-0">
          ...
          {activeTab === "members" && (
            <div className="space-y-4 page-enter">
              ...
              {membersLoading ? (
                ...
                <EmptyState icon={<Users className="w-5 h-5" />} title="Nenhum membro encontrado" description="Tente outro termo de busca." />
              ) : (
          ...
          {activeTab === "productions" && (
            <div className="space-y-4 page-enter">
              ...
              <Input
                placeholder="Buscar producao..."
                value={productionSearch}
                onChange={e => setProductionSearch(e.target.value)}
                className="max-w-xs"
                data-testid="input-production-search"
              />
              {prodsLoading ? (
                <div className="flex justify-center py-12">
                  <Loader2 className="w-6 h-6 animate-spin text-muted-foreground" />
                      ...
                      <div key={p.id} className="vhub-table-row" data-testid={`production-${p.id}`}>
                        ...
                        <div className="flex items-center gap-1">
                          <Button
                            ...
                            onClick={() => {
                              setEditProduction(p);
                              setEditProductionForm({ name: p.name, description: p.description || "", videoUrl: p.videoUrl || "", isPublic: p.isPublic || false, scriptJson: p.scriptJson || "" });
                            }}
              ...
              ) : productionSearch.trim() ? (
                <EmptyState icon={<Film className="w-5 h-5" />} title="Nenhuma producao encontrada" description="Tente outro termo de busca." />
              ) : (
                <EmptyState
          ...
          {activeTab === "sessions" && (
            <div className="space-y-4 page-enter">
              ...
              {sessionsLoading ? (
                ...
                <EmptyState icon={<Calendar className="w-5 h-5" />} title="Nenhuma sessao encontrada" description="Tente outro termo de busca." />
              ) : (
                <EmptyState
                      ...
                      <div className="divide-y divide-border/40">
                        {(group.takes || []).map((take: any) => (
                          <div key={take.id} className="flex items-center gap-4 px-4 py-3" data-testid={`take-${take.id}`}>
                            ...
                            <StatusBadge status={take.status || "pending"} />
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="vhub-field">
              <label className="flex items-center gap-2 cursor-pointer">
                <Checkbox checked={editProductionForm.isPublic || false} onCheckedChange={v => setEditProductionForm(f => ({ ...f, isPublic: !!v }))} data-testid="check-edit-production-public" />
                <span className="text-sm text-foreground">Producao Publica</span>
              </label>
            </div>
            <div className="vhub-field">
              <label className="vhub-field-label">Roteiro (JSON - Opcional)</label>
              <Textarea
                value={editProductionForm.scriptJson}
                onChange={e => setEditProductionForm(f => ({ ...f, scriptJson: e.target.value }))}
                placeholder='{"lines": [{"character": "Nome", "start": "00:00:05", "text": "Fala"}]}'
                rows={6}
                className="font-mono text-xs"
                data-testid="input-edit-production-script-json"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Edite o JSON do roteiro ou use a página de Produções para um editor visual
              </p>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!rejectConfirm} onOpenChange={v => { if (!v) setRejectConfirm(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Rejeitar Cadastro</DialogTitle>
            <DialogDescription>
              Deseja rejeitar o cadastro de {rejectConfirm?.user?.fullName || rejectConfirm?.user?.email}? Esta acao nao pode ser desfeita.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setRejectConfirm(null)}>Cancelar</Button>
            <Button
              variant="destructive"
              disabled={rejectMutation.isPending}
              onClick={() => rejectConfirm && rejectMutation.mutate(rejectConfirm.id)}
      ...
      <Dialog open={createProductionOpen} onOpenChange={setCreateProductionOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="vhub-field">
              <label className="vhub-field-label">URL do Video</label>
              <Input value={newProductionVideoUrl} onChange={e => setNewProductionVideoUrl(e.target.value)} placeholder="URL (opcional)" data-testid="input-production-video-url" />
            </div>
            <div className="vhub-field">
              <label className="flex items-center gap-2 cursor-pointer">
                <Checkbox checked={newProductionIsPublic} onCheckedChange={v => setNewProductionIsPublic(!!v)} data-testid="check-production-public" />
                <span className="text-sm text-foreground">Producao Publica</span>
              </label>
            </div>
            <div className="vhub-field">
              <label className="vhub-field-label">Roteiro (JSON - Opcional)</label>
              <Textarea
                value={newProductionScriptJson}
                onChange={e => setNewProductionScriptJson(e.target.value)}
                placeholder='{"lines": [{"character": "Nome", "start": "00:00:05", "text": "Fala"}]}'
                rows={6}
                className="font-mono text-xs"
                data-testid="input-production-script-json"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Cole o JSON do roteiro ou deixe vazio para adicionar depois na página de Produções
              </p>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={createSessionOpen} onOpenChange={setCreateSessionOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="vhub-field">
              <label className="vhub-field-label">Status Inicial</label>
              <Select value={newSessionStatus} onValueChange={setNewSessionStatus}>
