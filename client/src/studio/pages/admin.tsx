...
const ALL_STUDIO_ROLES = [
  { value: "studio_admin", label: "Admin do Estúdio" },
...
function ProductionsSection() {
  ...
  const [editForm, setEditForm] = useState({ name: "", description: "", videoUrl: "" });
  const [createOpen, setCreateOpen] = useState(false);
  const [createForm, setCreateForm] = useState({ studioId: "", name: "", description: "" });
  const [search, setSearch] = useState("");
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/admin/productions"] }); qc.invalidateQueries({ queryKey: ["/api/admin/stats"] }); setCreateOpen(false); setCreateForm({ studioId: "", name: "", description: "" }); toast({ title: "Producao criada" }); },
              ...
              <tbody>
                {filtered.map((prod: any) => (
                  <tr key={prod.id} className="border-b border-white/6 last:border-0 hover:bg-white/3" data-testid={`row-production-${prod.id}`}>
                    ...
                    <td className="p-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button size="icon" variant="ghost" onClick={() => { setEditProd(prod); setEditForm({ name: prod.name, description: prod.description || "", videoUrl: prod.videoUrl || "" }); }} data-testid={`button-edit-prod-${prod.id}`}>
                          <Pencil className="h-4 w-4" />
                        </Button>
