...
export async function requestMicrophone(
  ...
  try {
    ...
    console.error(`[Mic] getUserMedia failed for mode "${mode}" — ${name}: ${message}`);
    if (currentState && currentState.audioContext.state !== "closed") {
      console.log("[Mic] Keeping existing mic state as fallback");
      return currentState;
    }
    if (name === "NotAllowedError" || name === "PermissionDeniedError") {
      throw new Error("Permissao ao microfone negada. Permita o acesso nas configuracoes do navegador.");
    }
    if (name === "NotFoundError" || name === "DevicesNotFoundError") {
      throw new Error("Nenhum microfone encontrado. Conecte um microfone e tente novamente.");
    }
    if (name === "NotReadableError" || name === "TrackStartError") {
      throw new Error("O microfone esta em uso por outro aplicativo.");
    }
    throw new Error(`Erro ao acessar microfone: ${name} — ${message}`);
  }
