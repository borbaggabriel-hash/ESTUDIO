...
import { 
  ...
  Megaphone, Send, Radio, ChevronLeft, UserCheck, X, Clapperboard
} from 'lucide-react';
                      ...
                      >
                        ...
                        <div className="glass-panel rounded-3xl border border-emerald-200/60 bg-emerald-50/40 overflow-hidden">
                          <div className="w-full flex items-center justify-between px-6 py-4">
                            ...
                            >
                              <Plus className="w-3 h-3" /> Adicionar Diretor
                            </button>
                          </div>

                          {/* Add Diretor Modal — rendered via portal to escape framer-motion transform context */}
                        ...
                        {/* Outros Usuários (platform_owner + studio_admin) */}
                        {directors.length > 0 && (
                          <div className="glass-panel rounded-3xl border border-amber-200/60 bg-amber-50/40 overflow-hidden">
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-bold text-amber-800 uppercase tracking-wider">Administradores do Estudio</span>
                                  ...
                                  <div key={d.id} className="bg-white/60 border border-amber-200/40 rounded-2xl p-4 flex flex-col gap-2">
                                    ...
                                    <div className="flex items-center justify-between mt-1">
                                      ...
                                      {d.role === 'studio_admin' && (
                                        <button
                                          onClick={async () => {
                                            if (!window.confirm(`Remover ${d.fullName ?? d.email} como ADM Estudio?`)) return;
                                            try {
                                              await firebaseService.changeUserRole(d.id, 'dublador');
                                              toast.success('ADM Estudio rebaixado para Dublador.');
                                              const updated = await firebaseService.getDirectors();
                          ...
                          {filteredStudents.length === 0 ? (
                            ...
                            filteredStudents.map((student: any) => (
                              ...
                              >
                                ...
                                >
                                  ...
                                  <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span className="px-2.5 py-1 rounded-full text-xs font-bold shrink-0 bg-blue-50 text-blue-600">
                                ...
                                <button
                                  onClick={async () => {
                                    if (!window.confirm(`Excluir ${student.name ?? student.email}? O usuário ficará inativo.`)) return;
                                    try {
                                      await firebaseService.deleteStudentAccount(student.id);
                                      toast.success('Aluno excluído com sucesso.');
                                      const updated = await firebaseService.getAllStudents();
                                      setDraft((prev: any) => ({
                                        ...prev,
                                        students: ((updated as any[]) || []).map((s: any) => ({
                                          ...s,
                                          name: s.full_name || s.name || 'Sem Nome',
                                          avatar: s.avatar_url || s.avatar || `https://i.pravatar.cc/150?u=${s.id}`,
                                        })),
                                  ...
                                  className="mt-auto w-full text-xs text-red-500 hover:text-red-700 hover:bg-red-50 rounded-xl py-1.5 font-medium transition-colors border border-transparent hover:border-red-100 flex items-center justify-center gap-1"
                                >
