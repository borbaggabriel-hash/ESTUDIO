...
import {
  ...
  User, Clapperboard, ChevronRight, Loader2
} from 'lucide-react';
...
function StudioAccessButton() {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (!res.ok) { navigate('/hub-dub/login'); return; }
      const studios: any[] = await res.json();
      if (studios.length === 1) {
        navigate(`/hub-dub/studio/${studios[0].id}/dashboard`);
      } else {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className="group flex items-center gap-3 bg-gradient-to-r from-violet-600 to-purple-700 hover:from-violet-700 hover:to-purple-800 text-white px-5 py-4 rounded-2xl shrink-0 transition-all duration-200 shadow-md hover:shadow-violet-200 disabled:opacity-70"
    >
      {loading
      ...
      <div className="text-left">
        <p className="text-[10px] text-violet-200 uppercase tracking-wider leading-none mb-0.5">Área do Dublador</p>
